        ...
        implements ConnectorPageSourceProvider
{
    ...
    /**
     * If columns contain any dereferenced columns, returns the base columns and the transforms necessary to dereference them.
     */
    private static Optional<ColumnDereferenceInfo> getColumnDereferenceInfo(List<IcebergColumnHandle> columns)
    {
        requireNonNull(columns, "columns is null");
        ...
        List<IcebergColumnHandle> baseColumns = new ArrayList<>();
        List<FieldDereference> fieldDereferences = new ArrayList<>();
        Map<Integer, Integer> baseColumnIdToOrdinal = new HashMap<>();
        for (IcebergColumnHandle column : columns) {
            IcebergColumnHandle baseColumn = column.getBaseColumn();
            int baseColumnId = baseColumn.getId();
            Integer ordinal = baseColumnIdToOrdinal.get(baseColumnId);
            if (ordinal == null) {
                ordinal = baseColumns.size();
                baseColumns.add(column.getBaseColumn());
                baseColumnIdToOrdinal.put(baseColumnId, ordinal);
            }
            fieldDereferences.add(new FieldDereference(ImmutableList.<Integer>builder()
                    .add(ordinal)
                    .addAll(applyProjection(column, baseColumn))
                    .build()));
        }

        return Optional.of(new ColumnDereferenceInfo(baseColumns, fieldDereferences));
    }

    private record ColumnDereferenceInfo(List<IcebergColumnHandle> baseColumns, List<FieldDereference> fieldDereferences)
    {
        private ColumnDereferenceInfo
        {
            baseColumns = ImmutableList.copyOf(requireNonNull(baseColumns, "baseColumns is null"));
            fieldDereferences = ImmutableList.copyOf(requireNonNull(fieldDereferences, "fieldDereferences is null"));
        }
    }

    private record FieldDereference(List<Integer> path)
    {
        private FieldDereference
        {
